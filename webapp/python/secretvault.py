...
@app.route('/')
def index():
    #flask.session["email"]="eggie5@gmail.com"
    # flask.session['user_id']=1;
    #flask.session['address']="3651 arizonas tsa 92104"
    try:
        user_id = flask.session['user_id']#Return the item of d with key key. Raises a KeyError if key is not in the map.
    except KeyError:
        return index_html	
    else:
